...
static void layer_update_proc(Layer *layer, GContext *ctx) {
  ...
  graphics_fill_radial(ctx, center, radius - (6 * HOURS_RADIUS), radius, 0, DEG_TO_TRIGANGLE(minute_angle));

  // Hours are dots
  radius -= 3 * HOURS_RADIUS;
  for(int i = 0; i < 12; i++) {
    int hour_angle = (int)(((float)i / 12.0F) * 360.0F);
    GPoint pos = gpoint_from_polar(&center, radius, DEG_TO_TRIGANGLE(hour_angle));
